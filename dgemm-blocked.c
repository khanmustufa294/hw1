...
const char* dgemm_desc = "Updated simple blocked dgemm.";
...
static inline void matmul_unrolled(int lda, int K, double* a, double* b, double* c)
{
...
//Similar args as doblock
static inline void matmul_original(int lda, int K, double* a, double* b, double* c)
{
    ...
    //Runs through every column of 'a' (Every row of 'b')
    for (int i = 0; i < K; ++i)
    {
...
//We assume data is stored in col-major. If not, use it like matmul_4x4(B, A, C)
void matmul_4x4(double *A, double *B, double *C) {
    __m256d col[4], sum[4];
    ...
    /* For each column j of B */
        for (int j = 0; j < N; ++j)
        {
            ...
            for (int k = 0; k < K; ++k)
                ...
                //Multiplication and addition here
...
static inline void do_block_fast (int lda, int M, int N, int K, double* A, double* B, double* C)
{
    ...
    for (j = 0 ; j < N_floor; j += 4)
    {
        ...
        for (i = 0; i < M_floor; i += 4) {
            ...
            c = C + i + j*lda;
            matmul_original(lda, K, a_ptr, b_ptr, c);
        }
    }
